...
bool isPrime(int n){if(n == 1) return false;if(n == 2) return true;if(n%2 == 0) return false;int sq=sqrt(n);for(int i = 3; i<=sq; i += 2 )if(n%i == 0) return false;return true;}
...
int32_t main(){
    ...
    while(tc--){
        int n; I(n);
        if(n==1) P(1);
        else if(isPrime(n)) P(n);
        else{
            int a = primeFactorization(n);
            P(a);
        }
    }
    return 0;
